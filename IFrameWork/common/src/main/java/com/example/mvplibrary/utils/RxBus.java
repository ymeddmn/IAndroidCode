...
public class RxBus {
    ...
    public <T> Observable<T> register(@NonNull Object tag) {
        ...
        Logger.t(TAG,1).d("register %s, \t size %s",tag.toString(),subjectList.size());
        return subject;
    }
    ...
    public RxBus unregister(@NonNull Object tag,  @NonNull Observable<?> observable) {
        ...
        if (null != subjects) {
            ...
            if (isEmpty(subjects)) {
                subjectMapper.remove(tag);
                Logger.t(TAG).d("unregister %s, \t size %s",tag.toString(),subjects.size());
            }
        }
        return getInstance();
    }
    ...
    public void post(@NonNull Object tag, @NonNull Object content) {
        Logger.t(TAG).d("postEvent eventName %s",tag);
        List<Subject> subjectList = subjectMapper.get(tag);
        if (!isEmpty(subjectList)) {
            for (Subject subject : subjectList) {
                subject.onNext(content);
                Logger.t(TAG).d("onEvent eventName %s",tag);
            }
        }
    }
